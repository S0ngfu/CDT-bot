...
const getEmbed = async (interaction, data, userId) => {
	...
	if (data && data.length > 0) {
		for (const d of data) {
			if (d.error) {
				embed.addFields({ name: `${d.id} ${userId ? '' : `: ${d['employe_giver.name_employee']}`}`, value: `❌ Le Transfert de ${d.quantite.toLocaleString('fr')} farines pour ${d['employe_receiver.name_employee']} ne s'est pas effectué, enregistré le ${time(moment(d.timestamp, 'YYYY-MM-DD hh:mm:ss.S ZZ').unix(), 'F')}`, inline: false });
			}
			else if (d.done) {
				embed.addFields({ name: `${d.id} ${userId ? '' : `: ${d['employe_giver.name_employee']}`}`, value: `✅ Le transfert de ${d.quantite.toLocaleString('fr')} farines pour ${d['employe_receiver.name_employee']} a bien été effectué, enregistré le ${time(moment(d.timestamp, 'YYYY-MM-DD hh:mm:ss.S ZZ').unix(), 'F')}`, inline: false });
			}
			else {
				embed.addFields({ name: `${d.id} ${userId ? '' : `: ${d['employe_giver.name_employee']}`}`, value: `Transfert en attente de ${d.quantite.toLocaleString('fr')} farines pour ${d['employe_receiver.name_employee']}, enregistré le ${time(moment(d.timestamp, 'YYYY-MM-DD hh:mm:ss.S ZZ').unix(), 'F')}`, inline: false });
			}
		}
	}
