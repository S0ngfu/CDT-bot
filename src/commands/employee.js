...
const { Employee, Grossiste, Bill, BillModel, OpStock } = require('../dbObjects');
...
const employeeEmbed = async (employee, grossiste = [], nb_delivery = [], nb_stock = [], date_firing = null) => {
	...
	if (nb_stock.length === 4) {
		embed.addFields({
			name: 'Mises en stock',
			value: `S : ${nb_stock[0][0]?.qt_stock ? (nb_stock[0][0].qt_stock.toLocaleString('fr')) : 0}\n`
				+ `S-1 : ${nb_stock[1][0]?.qt_stock ? (nb_stock[1][0].qt_stock.toLocaleString('fr')) : 0}\n`
				+ `S-2 : ${nb_stock[2][0]?.qt_stock ? (nb_stock[2][0].qt_stock.toLocaleString('fr')) : 0}\n`
				+ `S-3 : ${nb_stock[3][0]?.qt_stock ? (nb_stock[3][0].qt_stock.toLocaleString('fr')) : 0}`,
			inline: true });
	}
	else {
		embed.addFields({
